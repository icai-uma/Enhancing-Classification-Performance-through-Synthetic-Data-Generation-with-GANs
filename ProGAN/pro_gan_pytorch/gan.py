...
class ProGAN:
    ...
    def train(
        ...
        for current_depth in range(start_depth, self.depth + 1):
            ...
            for epoch in range(1, epochs[depth_list_index] + 1):
                ...
                if (
                    ...
                    save_file = model_dir / f"depth_{current_depth}_epoch_{epoch}.pt"
                    # torch.save(self.get_save_info(gen_optim, dis_optim), save_file)
                    torch.save(self.gen_shadow.module.state_dict(), save_file)
        ...
        import imageio
        batch_size = 4
        num_generated_images = 50000
        image_writing_path = Path('fid_results')
        with torch.no_grad():
            for img_num in tqdm(range(0, num_generated_images, batch_size)):
                num_imgs = min(batch_size, num_generated_images - img_num)
                fixed_input = torch.randn(num_imgs, self.latent_size).to(self.device)
                samples = feedback_generator(fixed_input, current_depth, alpha).detach()
                samples = adjust_dynamic_range(
                    samples, drange_in=(-1.0, 1.0), drange_out=(0.0, 1.0)
                    )
                samples = samples.to('cpu', torch.uint8)

                for batch_num, gen_img in enumerate(samples, start=1):
                    imageio.imwrite(
                        image_writing_path / f"{img_num + batch_num}.png",
                        gen_img,
                    )
        print('Writing gen dataset completed ...')
